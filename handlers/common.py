...
async def initiate(message: types.Message):
    await message.answer(greetings, reply_markup=get_keyboard())
...
async def show_currency(call: types.CallbackQuery):
    ...
    await call.message.edit_text(get_currency.get_shops(), parse_mode=types.ParseMode.HTML, reply_markup=get_keyboard(1))
    await call.answer()


@dp.callback_query_handler(text="back")
async def get_back(call: types.CallbackQuery):
    await call.message.edit_text(greetings, reply_markup=get_keyboard())
